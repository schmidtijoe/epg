...
def semc_sequence(params: opt.SeqConfig = opt.SeqConfig()):
    ...
    # grad crushing
    crusher_moment = 1.0
    seq.add_event(evts.Grad(moment=crusher_moment, duration=grad_duration), start_time=esp / 2 - grad_duration - rf_duration / 2)

    logModule.debug("refocus")
    # refocusing
    seq.add_event(evts.RF(flip_angle=ref_fa, phase=ref_phase, duration=rf_duration), start_time=esp / 2 - rf_duration / 2)
    # grad crushing
