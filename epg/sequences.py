...
def make_tse_sequence(alpha, N, esp, use_y90=True, t1t2=(0, 0)):
    ...
    time = np.append(time, esp / 2 + np.array(
        (N - 1) * [0, esp / 2, esp / 2, esp, esp] + np.repeat(esp * np.arange(N - 1), 5)))
    grad = (2 * N - 1) * [1]
    rf = []
    if use_y90:
        rf.append((90, 90))
...
def make_he_sequence(phis, alphas, esp, t1t2=(0, 0)):
    ...
    time = np.array([0, esp / 2, esp / 2])
    time = np.append(time,
                     esp / 2 + np.array(2 * m * [0, esp / 2, esp / 2, esp, esp] + np.repeat(esp * np.arange(2 * m), 5)))
